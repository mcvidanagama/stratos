...
public class ClusterMonitorFactory {
	...
	private static VMServiceClusterMonitor getVMServiceClusterMonitor(Cluster cluster)
		...
		VMServiceClusterMonitor clusterMonitor =
				new VMServiceClusterMonitor(cluster.getServiceName(), cluster.getClusterId());
	...
	private static KubernetesServiceClusterMonitor getDockerServiceClusterMonitor(Cluster cluster)
		...
		//        KubernetesClusterContext kubernetesClusterCtxt = new KubernetesClusterContext
		// (kubernetesHostClusterID,
		//                cluster.getClusterId(), cluster.getServiceName(),  autoscalePolicy, minReplicas,
		// maxReplicas);

		KubernetesServiceClusterMonitor dockerClusterMonitor =
				new KubernetesServiceClusterMonitor(cluster.getServiceName(), cluster.getClusterId());
		...
		for (Member member : cluster.getMembers()) {
			...
			if (MemberStatus.Activated.equals(member.getStatus())) {
				if (log.isDebugEnabled()) {
					String msg = String.format("Active member loaded from topology and added to active member list, " +
					                           "%s",
					                           member.toString());
					log.debug(msg);
				}
				...
				if (log.isDebugEnabled()) {
					String msg =
							String.format("Pending member loaded from topology and added to pending member list, %s",
							              member.toString());
					log.debug(msg);
				}
