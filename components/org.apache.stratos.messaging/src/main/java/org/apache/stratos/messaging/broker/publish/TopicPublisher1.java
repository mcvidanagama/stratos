 ...
 * 
 */
public class TopicPublisher1 extends MessagePublisher {

	private static final Log log = LogFactory.getLog(TopicPublisher1.class);
	 ...
	 */
	TopicPublisher1(String aTopicName) {
		super(aTopicName);
		...
		synchronized (TopicPublisher.class) {
			...
			try {
				...
				if (topicSession != null) {
					topicSession.close();
					if (log.isDebugEnabled()) {
						log.debug(String.format("Topic publisher session closed: [topic] %s",
						                        getName()));
					}
				}
				if (connector != null) {
					connector.close();
					if (log.isDebugEnabled()) {
						log.debug(String.format("Topic publisher connector closed: [topic] %s",
						                        getName()));
					}
				}
	...
	private void doPublish(String message, Properties headers) throws Exception, JMSException {
		if (!initialized) {
			...
			initialized = true;
			if (log.isDebugEnabled()) {
				log.debug(String.format("Topic publisher connector initialized: [topic] %s",
				                        getName()));
			}
		}
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug(String.format("Message published: [topic] %s [header] %s [body] %s",
				                        getName(), (headers != null) ? headers.toString() : "null",
				                        message));
			}
		} finally {
			if (topicPublisher != null) {
	...
	private javax.jms.TopicPublisher createPublisher(TopicSession topicSession) throws Exception,
	                                                                           JMSException {
