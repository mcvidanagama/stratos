...
public class CartridgeSubscriptionUtils {
	...
	public static BasicPayloadData createBasicPayload(CartridgeInfo cartridgeInfo,
	                                                  String subscriptionKey, Cluster cluster,
	                                                  Repository repository, String alias,
	...
	// public static BasicPayloadData createBasicPayload (LBCategoryContext
	// lbCategoryContext) {
	//
	// BasicPayloadData basicPayloadData = new BasicPayloadData();
	...
	//
	// if (lbCategoryContext.getSubscriptionAlias() != null &&
	// !lbCategoryContext.getSubscriptionAlias().isEmpty()) {
	...
	static class TenantSubscribedEventPublisher implements Runnable {
		...
		public void run() {
			try {
				...
				TenantSubscribedEvent subscribedEvent =
				                                        new TenantSubscribedEvent(tenantId,
				                                                                  serviceName,
				                                                                  clusterIds);
				...
				if (log.isErrorEnabled()) {
					log.error(String.format("Could not publish tenant subscribed event: [tenant-id] %d [service] %s",
	...
	public static void publishTenantUnSubscribedEvent(int tenantId, String serviceName,
		...
		try {
			...
			TenantUnSubscribedEvent event =
			                                new TenantUnSubscribedEvent(tenantId, serviceName,
			                                                            clusterIds);
	...
	public static void validateCartridgeAlias(int tenantId, String cartridgeType, String alias)
		...
		if (isAliasTaken) {
			String msg =
			             "The alias " + alias +
			                     " is already taken. Please try again with a different alias.";
			log.error(msg);
	...
	public static boolean isAliasTaken(int tenantId, String alias) {
		...
		// return
		// (dataInsertionAndRetrievalManager.getCartridgeSubscription(tenantId,
		// alias) == null) ? false : true;
			...
			if (Constants.NO_LOAD_BALANCER.equals(name)) {
				...
				if (log.isDebugEnabled()) {
					log.debug("This cartridge refers to existing load balancers. " + "[Type] " +
					          serviceType + "[Referenced Cluster Ids] " + clusterIdsVal);
				}
				...
				for (String clusterId : clusterIds) {
					try {
						AutoscalerServiceClient.getServiceClient()
						                       .checkLBExistenceAgainstPolicy(clusterId,
				...
				if ("true".equals(value)) {
					...
					if (lbCartridgeInfo == null) {
						String msg =
						             "Please specify a LB cartridge type for the cartridge: " +
						                     serviceType + " as category: " +
						...
						for (DeploymentPolicy policy : lbCartridgeDepPolicies) {
							...
							if (deploymentPolicyName.equals(policy.getId())) {
								...
								if (!getAutoscalerServiceClient().checkDefaultLBExistenceAgainstPolicy(deploymentPolicyName)) {
									...
									// if LB cartridge definition has properties
									// as well, combine
				...
				if ("true".equals(value)) {
					...
					if (log.isDebugEnabled()) {
						log.debug("This cartridge uses a service aware load balancer. [Type] " +
						          serviceType);
					}
								...
								if (!getAutoscalerServiceClient().checkServiceLBExistenceAgainstPolicy(serviceType,
									...
									if (lbCartridgeInfo.getProperties() != null &&
										...
										lbProperties.setProperties(combine(lbCartridgeInfo.getProperties(),
										                                   new Property[] {
										                                                   lbRefProperty,
										                                                   loadBalancedServiceTypeProperty }));
									...
									// set a payload property for load balanced
									// service type
									Property payloadProperty = new Property();
									payloadProperty.setName("LOAD_BALANCED_SERVICE_TYPE"); // TODO:
									                                                       // refactor
									                                                       // hardcoded
									                                                       // name
	...
	private static Property[] combine(Property[] propertyArray1, Property[] propertyArray2) {
		...
		System.arraycopy(propertyArray2, 0, combinedProperties, propertyArray1.length,
		                 propertyArray2.length);
